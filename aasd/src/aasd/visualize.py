...
class CowVisualizer:
    ...
    def __init__(self, cows: List[CowAgent]) -> None:
        self.cows: List[CowAgent] = cows
    ...
    def format_location(self, location: Tuple[float, float]) -> str:
        """Format location tuple to string"""
        lat, lon = location
        return f"({lat:.4f}, {lon:.4f})"

    def format_health(self, health: str) -> str:
    ...
    def get_border_char(self, lat: float, lon: float, boundaries: Tuple[float, float, float, float]) -> str:
    ...
    def display(self) -> None:
        ...
        print("=" * 80)
        print("🐄 COW HERD TRACKING SYSTEM - Real-Time Visualization")
        ...
        for cow in self.cows:
            ...
            print(
                f"   Location: {self.format_location(cow.location)} {self.get_border_char(cow.location[0], cow.location[1], cow.boundaries)}"
            )
        ...
        print(
            "Knowledge: \033[92mGreen\033[0m = Up to date (matches current state) | \033[91mRed\033[0m = Outdated (state has changed)"
        )
